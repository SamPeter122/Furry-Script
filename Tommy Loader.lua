...
local UI = (function()
	...
	-- Re-asserted every frame: the script keeps building after we first detect it, so
	-- anything created or re-enabled during the hold gets caught too.
	local Held     = {}
	local HoldConn = nil

	local function HoldMenu()
		if HoldConn then return end
		-- RenderStepped fires immediately before the frame is drawn, so a menu created
		-- since the last frame is disabled before it can ever render. Polling on a
		-- timer let it show for a frame or two first.
		HoldConn = RunService.RenderStepped:Connect(function()
			for _, Child in hui:GetChildren() do
				...
				then
					Child.Enabled = false
					if not table.find(Held, Child) then
	...
	-- Start holding right away so the menu can never flash before we notice it
	HoldMenu()
	...
	-- The hold loop already scans every frame, so "menu exists" is simply "we caught one"

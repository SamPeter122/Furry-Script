...
local UI = (function()
	...
	function API.Progress(Amount)
		Started = true
		Tween(Value, {Value = Amount})
	end

	function API.Finish(Text)
...
if IsPF then
	...
	task.spawn(function()
		...
		-- Poll from the start so a fast load finishes immediately instead of
		-- waiting out a fixed timeline; the second stage only shows if it is slow.
		local Deadline = os.clock() + 30
		local Staged = false
		repeat
			task.wait(0.05)
			if not Staged and os.clock() - Start > 0.45 then
				Staged = true
		...
		if UI.MenuLoaded() then
			UI.Finish(("Loaded in %.2fs"):format(os.clock() - Start))
		else

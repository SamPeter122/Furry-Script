...
local UI = (function()
	...
	-- PF takes ~10s to load, so hold the card back rather than parking it on screen
	-- for the whole wait. If the script is ready first, it never appears at all.
	local ShowDelay = IsPF and 2 or 0
	...
	local function PlayEntrance()
		if not Alive or Done or Shown then return end
		Shown = true
	...
	if ShowDelay > 0 then
		task.delay(ShowDelay, PlayEntrance)
	else
		PlayEntrance()
	end
	...
	local function Unload()
		...
		-- Never became visible (script finished inside ShowDelay) -- just clean up.
		if not Shown then

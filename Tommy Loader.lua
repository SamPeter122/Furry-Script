...
if IsPF then
	...
	-- The PF payload has to execute on an actor thread, and that thread does not
	-- share globals with us, so LuaProt cannot call back into this UI. Drive it here.
	task.spawn(function()
		...
		repeat task.wait(0.15) until UI.MenuLoaded() or os.clock() > Deadline

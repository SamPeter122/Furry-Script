...
local UI = (function()
	...
	function API.MenuLoaded()
		...
		pcall(function()
			...
			for _, Child in Container:GetChildren() do
				if Child ~= Holder and Child:IsA("ScreenGui") and #Child.Name == 0 then
					Found = true
					break
				end
			end
	...
	function API.Step(StepOrTime)
		...
		elseif StepOrTime == 2 then
			...
			API.Finish(("Authenticated in %.2fs"):format(StepOrTime))
		end
	end
...
if IsPF then
	...
	task.spawn(function()
		...
		-- The PF script takes ~10s (it fetches its library over HTTP), so creep the bar
		-- asymptotically toward 0.92 instead of parking it. Poll fast so a quick load
		-- still finishes immediately rather than waiting out a fixed timeline.
		local Deadline = os.clock() + 45
		local Staged, NextCreep = false, 0
		repeat
			task.wait(0.05)
			local Elapsed = os.clock() - Start
			if not Staged and Elapsed > 0.45 then
				Staged = true
				UI.Status("Loading script")
			end
			if Staged and os.clock() >= NextCreep then
				NextCreep = os.clock() + 0.3
				UI.Progress(0.35 + 0.57 * (1 - math.exp(-Elapsed / 7)), 0.35)
			end
		until UI.MenuLoaded() or os.clock() > Deadline

		if UI.MenuLoaded() then
			UI.Finish(("Loaded in %.2fs"):format(os.clock() - Start), 0.45)
		else
			UI.Fail("Timed out waiting for script")
		end
	end)

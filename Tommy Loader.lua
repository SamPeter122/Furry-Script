...
getgenv().LP_CUSTOM_LOADER = (function()
	...
	local SweepStroke = Create("UIStroke", {
		Parent = Window,
		Name = "\0",
		Color = Theme.Text,
		LineJoinMode = Enum.LineJoinMode.Miter,
	...
	local SweepGradient = Create("UIGradient", {
		...
		Transparency = NumSequence{
			...
			NumSequenceKeypoint(0.44, 1),
			NumSequenceKeypoint(0.5, 0.4),
			NumSequenceKeypoint(0.56, 1),
			NumSequenceKeypoint(1, 1)
		},
	})
	...
	table.insert(Conns, RunService.RenderStepped:Connect(function(Delta)
		...
		SweepGradient.Offset = Vector2New((Clock * 0.45) % 2 - 1, 0)

		ShineX = ShineX + Delta * 250
		if ShineX > CW then ShineX = -60 end
		Shine.Position = UDim2New(0, ShineX, 0, 0)

		if not Done then
			Glow.ImageTransparency = 0.5 - (0.5 + 0.5 * math.sin(Clock * 2.6)) * 0.08
		end
	end))

	Tween(Window, {BackgroundTransparency = 0})
	Tween(WindowStroke, {Transparency = 0})
	Tween(SweepStroke, {Transparency = 0})
	Tween(Glow, {ImageTransparency = 0.5})
	Tween(Liner, {Size = UDim2New(1, -20, 0, 1), BackgroundTransparency = 0})
	Tween(BarTrack, {BackgroundTransparency = 0})
	Tween(BarStroke, {Transparency = 0})
	Tween(BarFill, {BackgroundTransparency = 0})
	Tween(Shine, {BackgroundTransparency = 0})
	Tween(Title, {TextTransparency = 0})
	Tween(Project, {TextTransparency = 0})
	Tween(Status, {TextTransparency = 0})
	Tween(Percent, {TextTransparency = 0})
	Tween(Value, {Value = 0.04})

	task.spawn(function()
		local Final = "Tomboy.Hook"
		local Chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz#$%&@*<>/|"
		local Length = #Final
		local Revealed, Step = 0, 0
		while Revealed < Length and Alive do
			local Out = Final:sub(1, Revealed)
			for _ = Revealed + 1, Length do
				local Pick = math.random(1, #Chars)
				Out = Out .. Chars:sub(Pick, Pick)
			end
			pcall(function() Title.Text = Out end)
			Step = Step + 1
			if Step % 2 == 0 then
				Revealed = Revealed + 1
			end
		...
		if Alive then
			pcall(function() Title.Text = Final end)
		end
	end)
	...
	local function Unload()
		Alive = false
		Tween(Window, {BackgroundTransparency = 1}, FadeSpeed)
		Tween(WindowStroke, {Transparency = 1}, FadeSpeed)
		Tween(SweepStroke, {Transparency = 1}, FadeSpeed)
		Tween(Glow, {ImageTransparency = 1}, FadeSpeed)
	...
	return function(StepOrTime)
		...
		elseif StepOrTime == 2 then
			...
			Tween(WindowStroke, {Color = Theme.Text}, 0.12)
			Tween(Glow, {ImageTransparency = 0.22}, 0.12)
			task.delay(0.14, function()
				if not Alive then return end
				Tween(WindowStroke, {Color = Theme.Accent}, 0.45)
